...
class Game:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:  # Left click
                    if self.scout and self.scout.x == x and self.scout.y == y:
                        self.selected = self.scout
                    elif self.fog[y][x] == 'C' and (not self.scout or (self.scout.x != x or self.scout.y != y)):
                    ...
                    # Build menu scout button
                        ...
                        if self.colony.metal >= 5 and not self.scout:
    ...
    def render(self):
        ...
        if self.show_build_menu:
            ...
            scout_text = self.font.render(f"Scout ({5 if not self.scout else 'Built'})", True, (0, 0, 0))
