...
class Scout:
    ...
    def move_to(self, x, y, game):
        ...
        if current_time - self.last_move < MOVE_TICK_RATE:
            return False
        # Check friendly unit collision and enemy ZOC
        ...
        adjacent_to_enemy = any(abs(self.x - ex) + abs(self.y - ey) == 1 for ex, ey in enemy_positions)
        ...
        if adjacent_to_enemy:
            ...
            if dx == 0 and dy == 0:  # Stay still
                return True
            if not (abs(dx) + abs(dy) == 1 and (dx > 0 and self.x < ex or dx < 0 and self.x > ex or dy > 0 and self.y < ey or dy < 0 and self.y > ey)):
                return False  # Can only move away
        for ex, ey in enemy_positions:
            if abs(x - ex) + abs(y - ey) == 1 and not adjacent_to_enemy:
            ...
            if 0 <= next_x < game.planet["width"] and 0 <= next_y < game.planet["height"] and (next_x, next_y) not in friendly_positions:
                if adjacent_to_enemy:
                    ...
                    if not (dx > 0 and self.x < ex or dx < 0 and self.x > ex or dy > 0 and self.y < ey or dy < 0 and self.y > ey for ex, ey in enemy_positions):
                        return
                for ex, ey in enemy_positions:
                    if abs(next_x - ex) + abs(next_y - ey) == 1 and not adjacent_to_enemy:
                        return
